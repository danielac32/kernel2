...
int load_elf(program_img *prg, exec_img *res,char *out) {
    ...
    for (int sec_idx = 0; sec_idx < hdr->e_shnum; ++sec_idx) {
        ...
        if (shdr->sh_flags & SHF_ALLOC && shdr->sh_size) { // non-empty allocatable sections
            ...
            //printf("%s %x %x %x\n",name,shdr->sh_addr,shdr->sh_size,shdr->sh_off);
              
              //printf("%s %x %x\n",name,shdr->sh_off,shdr->sh_size);
              hexDump (shdr->sh_addr,name,&prg->img[shdr->sh_off], shdr->sh_size);
             
            //shdr->sh_addr//desde  donde empieza
...
int main(int argc, char const *argv[])
{
    ...
    int r = load_elf(&prg1, ximg,"out.bin");
